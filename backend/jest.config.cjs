/**
 * Jest for Nest (TS + decorators): `*.integration.spec.ts` (DB) + `*.util.spec.ts` (pure unit) + `*.facade.spec.ts` (facade parity).
...
module.exports = {
  ...
  roots: ['<rootDir>/src'],
  testMatch: ['**/*.integration.spec.ts', '**/*.util.spec.ts', '**/*.facade.spec.ts'],

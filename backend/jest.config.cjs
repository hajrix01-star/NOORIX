/**
 * Jest for Nest (TS + decorators): `*.integration.spec.ts` (DB) + `*.util.spec.ts` (pure unit) + `*.facade.spec.ts` (facade parity) + `*.service.spec.ts` (service unit) + `*.controller.spec.ts` (controller unit) + `*.handler.spec.ts` (chat handlers).

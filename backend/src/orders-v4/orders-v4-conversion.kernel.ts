...
function assertUniqueUnits(units: readonly OrdersV4UnitDefinition[]): Map<string, OrdersV4UnitDefinition> {
  const byId = new Map<string, OrdersV4UnitDefinition>();
...
function authoredAdjacency(edges: readonly OrdersV4ConversionEdgeDefinition[]): Map<string, string[]> {
...
function assertNoAuthoredCycle(edges: readonly OrdersV4ConversionEdgeDefinition[]): void {
...
function buildTraversalEdges(
  unitsById: ReadonlyMap<string, OrdersV4UnitDefinition>,
...
export function validateOrdersV4ConversionDefinition(
...
export function resolveOrdersV4Conversion(input: {
  fromUnitId: string;
  toUnitId: string;
  units: readonly OrdersV4UnitDefinition[];
  edges: readonly OrdersV4ConversionEdgeDefinition[];
  ...
  if (!from || !to) throw new BadRequestException('الوحدة المطلوبة غير موجودة في كتالوج V4');
  if (from.id === to.id) {
    return { fromUnitId: from.id, toUnitId: to.id, factor: new Prisma.Decimal(1), source: 'identity', path: [] };
  }

  validateOrdersV4ConversionDefinition(input.units, input.edges);

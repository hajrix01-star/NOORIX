...
export type SaudiOccasionTemplate = {
  ...
  toDate: string;
  color: string;
  /** true = أم القرى (تقديري) — قد يُعدَّل بعد إعلان رسمي */
  estimated: boolean;
};
...
/** YMD بتوقيت الرياض من تاريخ هجري */
export function hijriToRiyadhYmd(hy: number, hm: number, hd: number): string {
...
function buildIslamicOccasionsForHijriYear(hy: number, gregorianYear: number): SaudiOccasionTemplate[] {
  ...
  const eidAdhaEndYmd = addGregorianDaysYmd(arafatYmd, 3);
...
export function listSaudiOccasionYears(): number[] {
  ...
  const to = y + 5;
  const years: number[] = [];
  for (let i = from; i <= to; i += 1) years.push(i);
  return years;
}

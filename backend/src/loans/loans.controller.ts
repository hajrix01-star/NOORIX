...
export class LoansController { constructor(private readonly service: LoansService) {}
  ...
  @Post(':id/legacy-expense-invoices') @RequirePermission('EXPENSES_WRITE') migrateLegacyInvoices(@Param('id') id: string, @CompanyId() companyId: string, @CurrentUser() user: JwtUser, @Body() dto: MigrateLoanLegacyInvoicesDto) { return this.service.migrateLegacyInvoices(id, companyId, dto, user.sub); }
  @Post(':id/legacy-expense-invoices/convert') @RequirePermission('EXPENSES_WRITE') convertLegacyInvoicesToPayments(@Param('id') id: string, @CompanyId() companyId: string, @CurrentUser() user: JwtUser) { return this.service.convertLegacyInvoicesToPayments(id, companyId, user.sub); }
}

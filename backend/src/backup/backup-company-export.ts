...
export async function buildCompanyLogicalSnapshot(
  ...
  const [
    ...
    employeeDeductions,
    payrollAdvanceSettlements,
    historicalPartTimePayrollLinks,
    fiscalPeriods,
    expenseLines,
    loans,
    loanPayments,
    ...
    prisma.payrollAdvanceSettlement.findMany({ where }),
    prisma.historicalPartTimePayrollLink.findMany({ where }),
    prisma.fiscalPeriod.findMany({ where }),
    prisma.expenseLine.findMany({ where }),
    prisma.loan.findMany({ where }),
    prisma.loanPayment.findMany({ where }),
    prisma.loanLegacyInvoice.findMany({ where }),
  ...
  const counts: Record<string, number> = {
    ...
    payrollAdvanceSettlements: payrollAdvanceSettlements.length,
    historicalPartTimePayrollLinks: historicalPartTimePayrollLinks.length,
    fiscalPeriods: fiscalPeriods.length,
    expenseLines: expenseLines.length,
    loans: loans.length,
    loanPayments: loanPayments.length,
    loanLegacyInvoices: loanLegacyInvoices.length,

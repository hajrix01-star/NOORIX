...
import { calculateOrdersV3StocktakeAdjustment } from './orders-v3-calculation.kernel';
import type { OrdersV3StocktakeInput } from './orders-v3.contracts';
...
function stocktakeRequestHash(input: OrdersV3StocktakeInput): string {
...
@Injectable()
export class OrdersV3InventoryService {
  constructor(private readonly prisma: TenantPrismaService) {}

  async balances(companyId: string) {
    const [entries, items, locations] = await Promise.all([
      this.prisma.ordersV3LedgerEntry.findMany({
        where: { companyId },
        include: { item: { include: { baseUnit: true, category: true } }, location: true },
      ...
      this.prisma.ordersV3Location.findMany({ where: { companyId, isActive: true }, orderBy: { nameAr: 'asc' } }),
    ]);
    ...
    const rows = [...latest.values()].map((entry) => ({
      ...
      unitCode: entry.item.baseUnit.code,
      unitName: entry.item.baseUnit.nameAr,
  ...
  async ledger(companyId: string, itemId?: string, locationId?: string) {
    const entries = await this.prisma.ordersV3LedgerEntry.findMany({
      where: { companyId, itemId: itemId || undefined, locationId: locationId || undefined },
      include: { item: { include: { baseUnit: true } }, location: true },
      orderBy: { sequence: 'desc' },
      take: 1000,
    });
  ...
  async listStocktakes(companyId: string) {
    return this.prisma.ordersV3Stocktake.findMany({
      where: { companyId },
  ...
  async createStocktake(companyId: string, input: OrdersV3StocktakeInput) {
    ...
    return this.prisma.withTenant(async (tx) => {
      const duplicate = await tx.ordersV3Stocktake.findFirst({
      ...
      const items = await tx.ordersV3Item.findMany({ where: { companyId, id: { in: itemIds }, isActive: true, trackInventory: true } });
      if (items.length !== itemIds.length) throw new BadRequestException('أحد أصناف الجرد غير صالح');
      for (const itemId of [...itemIds].sort()) {
        await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`orders-v3:ledger:${companyId}:${itemId}:${location.id}`}))`;
      }
      const stocktake = await tx.ordersV3Stocktake.create({
        data: {
          tenantId, companyId,
          stocktakeNumber: `STK3-${date.toISOString().slice(0, 10).replaceAll('-', '')}-${randomUUID().slice(0, 8).toUpperCase()}`,
      ...
      for (const line of [...input.lines].sort((a, b) => a.itemId.localeCompare(b.itemId))) {
        ...
        await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtext(${`orders-v3:ledger:${companyId}:${item.id}:${location.id}`}))`;
        ...
        const balance: OrdersV3InventoryBalance = {
        ...
        const calculation = calculateOrdersV3StocktakeAdjustment(balance, line.physicalQuantity);
        const createdLine = await tx.ordersV3StocktakeLine.create({
          data: {
            tenantId, companyId, stocktakeId: stocktake.id, itemId: item.id, unitId: item.baseUnitId,
        ...
        if (!calculation.quantityDelta.isZero()) {
          await tx.ordersV3LedgerEntry.create({
            data: {
              ...
              sourceSnapshot: { kernelVersion: 3, stocktakeNumber: stocktake.stocktakeNumber, stocktakeLineId: createdLine.id },
              createdByUserId: TenantContext.getUserId(),
            },
          });
        }
      }
      return tx.ordersV3Stocktake.findUniqueOrThrow({
  ...
  async dataQuality(companyId: string) {
    const items = await this.prisma.ordersV3Item.findMany({

...
## المرفقات والرفع (`uploads`)

الصور (OCR)، مستندات HR، والمرفقات تُكتب تحت **`backend/uploads/`** (أو `/app/uploads` داخل حاوية Docker). **لا تُترك داخل صورة الحاوية فقط** — عند إعادة النشر بدون volume تُفقد الملفات.

- **Docker / Railway:** الـ `Dockerfile` يعرّف `VOLUME ["/app/uploads"]`؛ اربط تخزيناً مستمراً من المنصة.
- **VPS (PM2 بدون Docker):** أنشئ مجلداً ثابتاً مثل `/var/lib/noorix/uploads` وربط رمزي أو `environment` يوجّه التطبيق لنفس المسار، أو انسخ احتياطياً قبل كل نشر. خطة **تخزين كائنات (S3/B2)** اختيارية لاحقاً.

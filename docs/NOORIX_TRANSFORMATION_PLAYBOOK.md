...
|---------|---------|--------|-------------------------|--------|---------------------|---------|
| 2026-04-26 | D | D (OCR: useInvoiceUploadTab) | `58481ba` | Cursor | ✓ | `useInvoiceUploadTab.js` يضم الاستعلامات والتأثيرات والتعامل مع الاستخراج/الحفظ؛ `InvoiceUploadTab.jsx` ≈100 سطر (عرض فقط). **التحقق:** vitest + vite build |
| 2026-04-26 | D | D (ملف موظف + تبويب رفع OCR) | `caa6701` | Cursor | ✓ | تقسيم `EmployeeProfileScreen` → `components/employeeProfile/*` + `employeeProfileModel`؛ `InvoiceUploadTab` → `components/invoiceUpload/*` (منطق الحالة يبقى في `InvoiceUploadTab`؛ بلا تغيير `export default`). **التحقق:** vitest + vite build |
...
| 2026-04-27 | E + تشغيل | إغلاق واجهة + إبطال HR + توثيق نشر | `c1d243b` | Cursor | ✓ | **الواجهة:** `strict: true` في `tsconfig.json`، `npm run typecheck`/`test`/`build`. **إبطال:** `queryInvalidation.ts` — بادئات HR (`employees`، `leaves`، `residencies`، …). **نشر:** قسم `uploads` في `DEPLOYMENT.md`. **A3:** لا ينطبق بدون تسرّب تاريخي. **خلفية:** خدمات ثقيلة (`financial-core…monolith` >450 سطر) تبقى تحت ميثاق «تقسيم عند التغيير» وليس حذفاً فورياً. |
| 2026-04-27 | C + QA | إكمال مراجعة 360 — P2 ووثائق | — | Cursor | ✓ | **`.gitignore`:** `cursorrules*.txt` بجانب `build-out.txt`. **`financial-core-helpers.util.spec.ts`:** توازن عشري 0.1+0.2=0.3 وسيناريو تحويل 1:1. **`NOORIX_360_REVIEW_ABRIL_2026.md`:** محاذاة §1 (T1–T3، فهارس `LedgerEntry`)؛ §3 — P2-2 إلى P2-4. **الاختبارات:** `npm test` (backend، `*.util.spec.ts`). |
| 2026-04-27 | B + QA | تدقيق P1-4 + توثيق الميزان | — | Cursor | ✓ | **الكود:** JSDoc على `validateJournalBalance` يذكر outflow / inflow / transfer. **الوثيقة 360:** إغلاق P1-4 (☑)؛ تصحيح §1 للصفوف 3–5 و7 و10 (prisma، extractJson، HttpException، JWT، منطق التوازن). |
| 2026-04-27 | E + P3 | إغلاق وثيقة 360 بالكامل | — | Cursor | ✓ | **مالي:** `syncActiveLedgerTransactionDateForOutflowInvoice` في `financial-outflow` + `FinancialCoreService`؛ استدعاء من `invoice.service`. **داشبورد:** `DashboardCalendarTab` + `DashboardAppSalesTab` → `useDashboardSalesPack`. **تواريخ:** `toYmd` في `saudiDate.ts` + `sales-summaries` API. **Gemini:** حد تزامن `GEMINI_MAX_CONCURRENT`. **تدقيق:** `scripts/audit-http-surface.mjs` + `npm run audit:http-surface`. **نسخ:** تعليق مسار `ledgerEntry` في `backup-logical-import`. **الوثيقة:** §1–§4 + P3-1…P3-4. **التحقق:** `nest build`، vitest `saudiDate`، jest util. |
| 2026-04-27 | C | `toYmd` — بقية apiEndpoints | — | Cursor | ✓ | **`toYmd`:** `invoices.ts`، `connection-bank.ts`، `reports.ts`، `accounts-categories-expense.ts`، `hr.ts`؛ §3 **P2-5**؛ تحديث بند §1 #8. **التحقق:** `npm run typecheck`. |
| 2026-04-27 | C | `toYmd` — واجهات وروابط | — | Cursor | ✓ | داشبورد (4 مكوّنات)، `PurchasesBatchScreen`، `PaymentHistoryTab`، `LeaveTab`/`LeaveFormModal`، `assetsRegisterUtils`، `ledgerInvoiceLink`، `payrollSalaryInvoiceHref`؛ تحديث **P2-5** في 360. **التحقق:** `npm run typecheck`. |
| 2026-04-27 | C | `toYmd` — واجهة ثانية + Nest | — | Cursor | ✓ | **واجهة:** `ReportsDetailModal`، `PayrollTab`، `EmployeeDocModal` (عقد)، `importTemplates`، `HrPrintDocumentsTab`/`hrPrintDocumentsTabFormat`، كشوف بنك (`export/print`، `AnalysisScreen`، `MappingModal`، `SummaryCards`، `SalesCompareTab`)، `useBankStatementView`، `DayCloseReportModal`، `DashboardSpecialDaysTab`، OCR (`useInvoiceUploadTab`، `OcrLinkedPurchaseForm`)، `OwnerDashboardScreen`، `ResidencyFormModal`، `employeeProfileModel`، `EOSCalcTab`. **خادم:** `common/utils/to-ymd.util.ts` + استبدالات عبر الخدمات أعلاه. **التحقق:** `npm run typecheck` (جذر) + `npm run build` (backend). |
| 2026-04-27 | C | `toYmd` — اختبارات وحدة | — | Cursor | ✓ | **`backend`:** `to-ymd.util.spec.ts` (jest). **الواجهة:** `saudiDate.test.ts` — حالة `trim` لـ `toYmd`. **`sales-summary-date-range`:** `addDaysYmd` يعيد `toYmd(dt)` بدل `toISOString().slice`. **التحقق:** `npx jest to-ymd.util.spec.ts`، `npx vitest run src/utils/saudiDate.test.ts`. |
| 2026-04-27 | C | `toYmd` — مواءمة `Date` + توثيق | — | Cursor | ✓ | **الواجهة:** `toYmd` يدعم `instanceof Date` (UTC، مثل الخادم) + vitest. **JSDoc:** `saudiDate.ts` → `@see toDateInputYmd` للرياض؛ `to-ymd.util.ts` → إشارة لـ `toDateInputYmd` للواجهة. **وثيقة 360:** §1 #8 + **P2-5** — تمييز UTC مقابل الرياض. **سكربت:** `test-advance-via-core.js` — `toYmd` محلي + إزالة `require` مكرر. |
| 2026-04-27 | — | **إغلاق مسار `toYmd`** | — | Owner | ✓ | **مغلق:** لا عمل نشط على المسار. **الخطوة التالية (مقترحة):** ميثاق ≤450 سطر وتقسيم الخدمات الثقيلة عند التغيير؛ [FRONTEND_CENTRALIZATION_CHECKLIST.md](FRONTEND_CENTRALIZATION_CHECKLIST.md)؛ اختياري Sentry/S3 — انظر `NOORIX_360_REVIEW_ABRIL_2026.md` «ما بعد إغلاق 360». |
| 2026-04-27 | F | **إبطال كاش — بنك + مالك** | — | Cursor | ✓ | **`queryInvalidation.ts`:** إضافة بادئات `bank-statements` / `bank-statement` / `bank-reconciliation-stats` / تصنيف الكشف / قوالب؛ `owner-daily-sales`. **`PERFORMANCE_AND_DATA.md`:** تحديث وصف الإبطال ومسار `.ts`. **التحقق:** `npm run typecheck`. |
| 2026-04-27 | F | **إبطال كاش — قنوات + ضريبة + طلبات + OCR** | — | Cursor | ✓ | **`queryInvalidation.ts`:** `sales-channels`، `vat-planning`، `order-products`، `order-categories`، `ocr-invoices`، `ocr-purchases-report`، `ocr-suppliers`، `ocr-accounting-supplier-suggestions`، `accounting-suppliers-ocr-catalog`. **`TOAST_AND_API_MUTATIONS.md`:** مسار `.ts`. **التحقق:** `npm run typecheck`. |
| 2026-04-27 | F | **إبطال كاش — فئات + شركة + خزائن دفع + OCR كامل** | — | Cursor | ✓ | **`queryInvalidation.ts`:** `payment-vaults`، `categories`، `company`، `ocr-items`، `ocr-price-alerts`، `ocr-review-queue`. **`PERFORMANCE_AND_DATA.md`:** تحديث السطر. **التحقق:** `npm run typecheck`. |
| 2026-04-27 | F | **إبطال كاش — جرد مفاتيح 100% (OCR كتالوج)** | — | Cursor | ✓ | **`queryInvalidation.ts`:** `ocr-catalog-accounting-suggestions`؛ تعليق استثناءات (`me`، `companies`، `backup-*`، `ocr-invoice-image`، …). **`PERFORMANCE_AND_DATA.md`:** ذكر المفتاح ضمن OCR. **التحقق:** `npm run typecheck`. |
| 2026-04-27 | Split | **financial-inflow ≤450** | — | Cursor | ✓ | **`financial-inflow-channels.util.ts`** + **`financial-inflow-ledger.util.ts`** + **`financial-inflow-channels.util.spec.ts`**؛ تقليل **`financial-inflow.service.ts`** تحت ميثاق 450؛ تحذير إضافي في **`assemble-financial-core.mjs`**. **التحقق:** `npm run check:service-footprint` + jest (`financial-inflow-channels`، `financial-core-helpers`) + `nest build`. |
| 2026-04-27 | Split | **financial-inflow ≤450** | — | Cursor | ✓ | **`financial-inflow-channels.util.ts`** + **`financial-inflow-ledger.util.ts`** + **`financial-inflow-channels.util.spec.ts`**؛ **`financial-inflow.service.ts`** تحت ميثاق 450؛ توسيع تحذير **`assemble-financial-core.mjs`**. **التحقق:** `check:service-footprint` + jest + `nest build`. |
| 2026-04-27 | Ops | **تقرير المتبقي 1–10 — تنفيذ** | — | Cursor | ✓ | **`financial-outflow-ledger.util.ts`** + اختبار؛ **`HttpExceptionFilter`** إخفاء 5xx لـ HttpException في prod؛ **`scripts/verify-git-cleanliness.mjs`** + **`npm run verify:git-cleanliness`**؛ **`backend/scripts/check-heavy-service-footprint.mjs`** + **`npm run check:service-footprint`**؛ تحذير في **`assemble-financial-core.mjs`**؛ وثائق **`REPOSITORY_MAINTENANCE_RUNBOOK.md`**، **`RLS_APPLY_RUNBOOK.md`**، **`EXECUTION_PLAN_17_GAP_AUDIT.md`**؛ بند صيانة في **`FRONTEND_CENTRALIZATION_CHECKLIST.md`**؛ **`backend/.env.example`** (Sentry traces + S3 endpoint). **التحقق:** `npm run typecheck` (جذر) + `npm test` (jest financial-outflow-ledger + financial-core-helpers) + `nest build` (backend). |

...
|---------|---------|--------|-------------------------|--------|---------------------|---------|
| 2026-04-26 | C | C3 + C5 + C6 | `154b8f6` | Cursor | ✓ | `@CompanyId` على `suppliers` / `ledger` / `chat` (مع بقاء fallback لأول شركة في الشات)؛ `assert-vaults-for-payment.util` + `hr.service`؛ `imageUtils.compressImageFileToJpegDataUrl` وOCR تبويب الرفع. **الاختبارات:** vitest + vite build + nest build |

import React, { forwardRef } from 'react';
import { cn } from './cn';

export type InlineSelectProps = React.ComponentPropsWithoutRef<'select'> & {
  invalid?: boolean;
};

const InlineSelect = forwardRef<HTMLSelectElement, InlineSelectProps>(function InlineSelect(
  { className = '', invalid = false, children, ...rest },
  ref,
) {
  return (
    <select
      ref={ref}
      aria-invalid={invalid || undefined}
      className={cn(
        'h-8 min-w-0 rounded border border-noorix-border bg-noorix-surface px-2 text-[12px] text-noorix-text',
        'focus:outline-none focus:border-noorix-blue focus:ring-1 focus:ring-noorix-blue/30',
        'disabled:cursor-not-allowed disabled:bg-noorix-bg-muted disabled:opacity-60',
        invalid && 'border-noorix-red focus:border-noorix-red focus:ring-noorix-red/30',
        className,
      )}
      {...rest}
    >
      {children}
    </select>
  );
});

export default InlineSelect;

...
function MetricCard({ color, isArchived = false, onClick, className, children }) {
  ...
  return (
    <div
      className={cn(
        'noorix-surface-card relative flex min-w-0 flex-col',
    ...
    >
      ...
      <div
        className="h-1 shrink-0 rounded-t-[var(--noorix-card-radius)]"
 ...
 * وضع KPI (بلا أيقونة):  label فقط أعلى اليسار
 ════════════════════════════════════════════════════════════════════ */
MetricCard.Header = function MetricCardHeader({ label, subLabel, icon, actions, className }) {
  if (icon || actions) {
    return (
 ...
 * grow: true في كروت KPI لدفع التذييل للأسفل (flex-1)
 ════════════════════════════════════════════════════════════════════ */
MetricCard.Spark = function MetricCardSpark({ data = [], color, height = 36, grow = false, className }) {
  return (
...
/* ══ Divider ══════════════════════════════════════════════════════════ */
MetricCard.Divider = function MetricCardDivider({ className }) {
 ...
 * شريط أفقي في أسفل الكرت — المتصل يُضيف py/border عبر className
 ════════════════════════════════════════════════════════════════════ */
MetricCard.Footer = function MetricCardFooter({ children, className }) {
  return (
 ...
 * قسم عام بحشو px-4 — للمحتوى الخاص (وارد/صادر في الخزينة…)
 ════════════════════════════════════════════════════════════════════ */
MetricCard.Section = function MetricCardSection({ children, className }) {

/**
 * ترجمات لوحة التحكم
 */
export default {
  dashboardDesc: { ar: 'نظرة عامة على الأعمال، التقويم، ومبيعات التطبيق.', en: 'Business overview, calendar, and app sales.' },
  dashboardOverview: { ar: 'نظرة عامة', en: 'Overview' },
  dashboardCalendar: { ar: 'التقويم', en: 'Calendar' },
  dashboardAppSales: { ar: 'مبيعات التطبيق', en: 'App Sales' },
  dashboardPlaceholder: { ar: 'سيتم إضافة المحتوى قريباً.', en: 'Content will be added soon.' },
  dashboardSalesTimeline: { ar: 'خط زمني للمبيعات', en: 'Sales timeline' },
  dashboardTimelineDaily: { ar: 'يومي', en: 'Daily' },
  dashboardTimelineMonthly: { ar: 'شهري', en: 'Monthly' },
  dashboardSalesTarget: { ar: 'هدف المبيعات', en: 'Sales target' },
  dashboardSetTarget: { ar: 'تعيين الهدف', en: 'Set target' },
  dashboardTargetAchieved: { ar: 'تحقق الهدف', en: 'Target achieved' },
  dashboardAppSalesRatio: { ar: 'نسبة مبيعات التطبيقات', en: 'App sales ratio' },
  dashboardAppSalesChart: { ar: 'نسبة التطبيقات من المبيعات — سنوي', en: 'App share of sales — annual' },
  dashboardTargetOverall: { ar: 'الهدف الشامل للشهر', en: 'Overall month target' },
  dashboardTargetByDay: { ar: 'هدف لكل يوم (أسبوعي)', en: 'Target per weekday' },
  dashboardTargetAllMonths: { ar: 'لكل الشهور', en: 'All months' },
  dashboardTargetThisMonthOnly: { ar: 'هذا الشهر فقط', en: 'This month only' },
  dashboardTargetScopeLabel: { ar: 'حفظ الهدف', en: 'Save target' },
  dashboardTargetDefault: { ar: 'الهدف الافتراضي — لكل الشهور', en: 'Default — applies to all months' },
  dashboardTargetMonthSpecific: { ar: 'مخصص لهذا الشهر', en: 'Custom for this month' },
  dashboardTargetResetToDefault: { ar: 'إعادة للافتراضي', en: 'Reset to default' },
  dashboardTargetDefaultNote: { ar: 'الأشهر الأخرى تستخدم الهدف الافتراضي العام', en: 'Other months use the global default target' },
  dashboardSpecialDays: { ar: 'أيام خاصة', en: 'Special days' },
  dashboardSpecialDaysHint: { ar: 'اضغط على نوع اليوم لتمييزه (إجازة / رمضان)', en: 'Click day type to mark (holiday / Ramadan)' },
  dashboardSpecialDaysFromTo: { ar: 'من تاريخ — إلى تاريخ', en: 'From date — To date' },
  dashboardSpecialDayName: { ar: 'اسم الفترة (مثلاً: رمضان، عيد الفطر)', en: 'Period name (e.g. Ramadan, Eid)' },
  dashboardSpecialDay: { ar: 'فترة خاصة', en: 'Special period' },
  dashboardNoSpecialDays: { ar: 'لا توجد أيام خاصة. اضغط «إضافة» للبدء.', en: 'No special days. Click «Add» to start.' },
  dashboardSpecialDaysDesc: { ar: 'أضف فترات خاصة مثل رمضان أو الأعياد لتظهر مميزة في التقويم.', en: 'Add special periods like Ramadan or holidays to highlight them on the calendar.' },
  dashboardHoliday: { ar: 'إجازة', en: 'Holiday' },
  dashboardRamadan: { ar: 'رمضان', en: 'Ramadan' },
  dashboardLegendGray: { ar: 'رمادي: أيام بدون مبيعات', en: 'Gray: days with no sales' },
  dashboardLegendRed: { ar: 'أحمر: أقل من 80٪ من الهدف اليومي', en: 'Red: below 80% of daily target' },
  dashboardLegendYellow: { ar: 'أصفر/كهرماني: من 80٪ إلى أقل من 100٪ من الهدف', en: 'Amber: 80%–99% of daily target' },
  dashboardLegendGreen: { ar: 'أخضر: حقق الهدف (100٪–119٪)', en: 'Green: target met (100%–119%)' },
  dashboardLegendBlue: { ar: 'أزرق: تجاوز الهدف بـ20٪ أو أكثر (≥120٪)', en: 'Blue: ≥120% of target (20%+ above goal)' },
  dashboardLegendGreenNoTarget: { ar: 'أخضر فاتح: مبيعات بدون هدف محدد', en: 'Light green: sales with no target set' },
  dashboardLegendSpecial: { ar: 'ملون: أيام خاصة (رمضان، أعياد)', en: 'Colored: special days (Ramadan, holidays)' },
  dashboardDayNote: { ar: 'ملاحظة اليوم', en: 'Day note' },
  dashboardDayNotePlaceholder: { ar: 'أضف ملاحظة لهذا اليوم...', en: 'Add a note for this day...' },
  dashboardAddAsSpecialDays: { ar: 'إضافة كأيام خاصة', en: 'Add as special days' },
  dashboardSelectDaysHint: { ar: 'اختر أياماً ثم اضغط لإضافتها كأيام خاصة (Shift+نقرة لتحديد نطاق)', en: 'Select days then click to add as special days (Shift+click for range)' },
  dashboardSelectedDays: { ar: 'أيام محددة', en: 'Selected days' },
  dashboardSelectDaysMode: { ar: 'تحديد أيام', en: 'Select days' },
  dashboardSelectDaysModeOff: { ar: 'إلغاء التحديد', en: 'Cancel selection' },
  dashboardNoAppSales: { ar: 'لا توجد مبيعات عبر التطبيقات في هذه الفترة', en: 'No app sales in this period' },
  dashboardKpiFormulaSales: { ar: 'إجمالي المبيعات للفترة', en: 'Total sales for the period' },
  /** متوسط يومي: المجموع ÷ عدد الأيام ذات الإيراد &gt; 0 فقط */
  dashboardSalesDailyAvgActiveDays: { ar: 'المعدل اليومي', en: 'Daily average' },
  /** فئات الموردين — نفس فترة التحليل */
  dashboardSuppliersInPeriod: { ar: 'موردون بحركة في الفترة', en: 'Suppliers with activity in period' },
  dashboardNoSuppliersInPeriod: { ar: 'لا يوجد موردون بفواتير مشتريات/مصروف في هذه الفترة.', en: 'No suppliers with purchase/expense invoices in this period.' },
  /** مشتريات حسب فئة الفاتورة — لوحة التحكم */
  dashboardPurchasesByCategoryTitleMonth: { ar: 'مشتريات الفئات لهذا الشهر', en: 'Purchases by category this month' },
  dashboardPurchasesByCategoryTitlePeriod: { ar: 'مشتريات الفئات للفترة', en: 'Purchases by category for the period' },
  dashboardPurchasesTotalForPeriod: { ar: 'إجمالي المشتريات للفترة', en: 'Total purchases for the period' },
  dashboardNoPurchasesByCategory: { ar: 'لا توجد مشتريات مصنّفة حسب الفئة في هذه الفترة.', en: 'No categorized purchases in this period.' },
  /** دمج الفئات السادسة فما فوق في المخطط والقائمة */
  dashboardSupplierCategoriesOthers: { ar: 'أخرى (فئات إضافية)', en: 'Other (additional categories)' },
  dashboardPurchasesByCategoryOthers: { ar: 'أخرى (فئات)', en: 'Other (categories)' },
  dashboardKpiFormulaPurchases: { ar: 'مشتريات الفترة', en: 'Purchases for the period' },
  dashboardKpiFormulaExpenses: { ar: 'مصروفات الفترة', en: 'Expenses for the period' },
  dashboardKpiFormulaGrossProfit: { ar: 'مبيعات − مشتريات', en: 'Sales − purchases' },
  dashboardKpiFormulaNetProfit: { ar: 'مبيعات − مشتريات − مصروفات', en: 'Sales − purchases − expenses' },
  dashboardKpiPctSales: { ar: 'نسبة المبيعات من المبيعات', en: 'Sales share of total sales' },
  dashboardKpiPctGrossProfit: { ar: 'نسبة الربح الإجمالي من المبيعات', en: 'Gross profit share of sales' },
  dashboardKpiPctNetProfit: { ar: 'نسبة الربح الصافي من المبيعات', en: 'Net profit share of sales' },

  /** رؤى لوحة التحكم — قراءة فقط */
  dashboardInsightsSectionTitle: { ar: 'تنبيهات وتحليلات ذكية', en: 'Smart Insights' },
  dashboardInsightsLoadingHint: { ar: 'جاري تحميل التحليلات…', en: 'Loading insights…' },
  dashboardInsightsSeverityInfo: { ar: 'معلومة', en: 'Info' },
  dashboardInsightsSeverityWarning: { ar: 'تنبيه', en: 'Warning' },
  dashboardInsightsSeverityCritical: { ar: 'حرج', en: 'Critical' },
  dashboardInsightsBasisAccounting: { ar: 'محاسبة (أرباح وخسائر)', en: 'Accounting P&L' },
  dashboardInsightsBasisOperational: { ar: 'مبيعات تشغيلية', en: 'Operational sales' },
  dashboardInsightsBasisInvoicePeriod: { ar: 'فواتير الفترة', en: 'Invoice period' },
  dashboardInsightsEmptyTitle: {
    ar: 'لا توجد تنبيهات مالية حالياً',
    en: 'No financial alerts',
  },
  dashboardInsightsEmptyDetail: {
    ar: 'الأرقام الحالية لا تتجاوز حدود التحذير المعتمدة.',
    en: 'Current figures do not exceed the configured warning thresholds.',
  },

  /** تكامل رؤى KPI — تذييل نسبة فقط (القيمة الملونة) */
  dashboardKpiInsightPurchasesAboveWarn: {
    ar: '{base}% — أعلى من الحد {limit}%',
    en: '{base}% — above limit {limit}%',
  },
  dashboardKpiInsightPurchasesAboveCrit: {
    ar: '{base}% — خطر: أعلى من الحد {limit}%',
    en: '{base}% — critical: above limit {limit}%',
  },
  dashboardKpiInsightExpensesAboveWarn: {
    ar: '{base}% — أعلى من الحد {limit}%',
    en: '{base}% — above limit {limit}%',
  },
  dashboardKpiInsightExpensesAboveCrit: {
    ar: '{base}% — خطر: أعلى من الحد {limit}%',
    en: '{base}% — critical: above limit {limit}%',
  },
  dashboardKpiInsightNetMarginBelowWarn: {
    ar: '{base}% — أقل من الحد {limit}%',
    en: '{base}% — below limit {limit}%',
  },
  dashboardKpiInsightNetMarginBelowCrit: {
    ar: '{base}% — خطر: أقل من الحد {limit}%',
    en: '{base}% — critical: below limit {limit}%',
  },
  dashboardKpiInsightNetProfitNegative: { ar: 'صافي الربح سلبي', en: 'Negative net profit' },
  dashboardKpiInsightPurchasesUnusuallyHigh: {
    ar: 'أعلى من المعتاد +{pct}%',
    en: 'Above normal +{pct}%',
  },
  dashboardKpiFooterNetProfitMarginLabel: { ar: 'هامش صافي الربح', en: 'Net profit margin' },

  dashboardStudioNav: { ar: 'لوحة التحكم (استوديو)', en: 'Dashboard (Studio)' },
  dashboardStudioTitle: { ar: 'استوديو لوحة التحكم', en: 'Dashboard studio' },
  dashboardStudioBeta: { ar: 'تجريبي', en: 'Beta' },
  dashboardStudioDesc: {
    ar: 'نظرة عامة + ملخص فواتير الفترة + رؤى وصحة مالية + مقارنة بالفترة السابقة — مع فلاتر إضافية وتصدير موسّع.',
    en: 'Overview plus period invoice mix, financial insights & health, prior-period compare — extra filters and richer export.',
  },
  dashboardStudioClassicLink: { ar: 'اللوحة التقليدية', en: 'Classic dashboard' },
  dashboardStudioExportCsv: { ar: 'تصدير KPI (CSV)', en: 'Export KPIs (CSV)' },
  dashboardStudioFilters: { ar: 'الفلاتر', en: 'Filters' },
  dashboardStudioFiltersHint: {
    ar: 'السنة والشهر يحددان تقرير الأرباح والخسائر وتحليل الفترة. الخيارات أدناه تؤثر على الرؤى والتصدير.',
    en: 'Year and month drive P&L and period analytics. Options below affect insights and export.',
  },
  dashboardStudioAdvancedFilters: { ar: 'فلاتر تحليل', en: 'Analysis options' },
  dashboardStudioIncludeCancelled: { ar: 'تضمين المبيعات الملغاة في الرؤى', en: 'Include cancelled sales in insights' },
  dashboardStudioIncludeCancelledHint: {
    ar: 'يغيّر طلب الرؤى الذكية فقط؛ أرقام بطاقات KPI تبقى من دفتر الأستاذ كما في النظام.',
    en: 'Changes the insights API only; KPI card figures stay ledger-based like the rest of the app.',
  },
  dashboardStudioIncludeCancelledNote: { ar: 'تضمين_ملغاة', en: 'include_cancelled' },
  dashboardStudioCancelledBadge: { ar: 'رؤى + ملغاة', en: 'Insights + cancelled' },
  dashboardStudioCompareToggle: { ar: 'مقارنة بالفترة السابقة', en: 'Compare to previous period' },
  dashboardStudioCompareToggleHint: {
    ar: 'شهر: مقارنة بالشهر السابق في نفس السنة. سنة كاملة: مقارنة بالسنة الميلادية السابقة.',
    en: 'Month: vs previous month same year. Full year: vs previous calendar year.',
  },
  dashboardStudioPeriodSnapshotTitle: { ar: 'ملخص فواتير الفترة', en: 'Period invoice snapshot' },
  dashboardStudioByInvoiceKind: { ar: 'حسب نوع الفاتورة', en: 'By invoice kind' },
  dashboardStudioTopSuppliersShort: { ar: 'أعلى موردين (مشتريات/مصروف)', en: 'Top suppliers (purchases/expense)' },
  dashboardStudioInsightsTitle: { ar: 'الرؤى والصحة المالية', en: 'Insights & financial health' },
  dashboardStudioHealthTitle: { ar: 'مؤشر الصحة', en: 'Health score' },
  dashboardStudioMetricsAccounting: { ar: 'مقاييس محاسبية (الفترة)', en: 'Accounting metrics (period)' },
  dashboardStudioRatiosTitle: { ar: 'نسب رئيسية', en: 'Key ratios' },
  dashboardStudioSalesBreakdown: { ar: 'تفصيل المبيعات (شهر)', en: 'Sales breakdown (month)' },
  dashboardStudioShare: { ar: 'حصّة', en: 'Share' },
  dashboardStudioOpportunities: { ar: 'فرص', en: 'Opportunities' },
  dashboardStudioWarnings: { ar: 'تنبيهات', en: 'Warnings' },
  dashboardStudioOtherInsights: { ar: 'رؤى', en: 'Insights' },
  dashboardStudioActiveSalesDays: { ar: 'أيام بمبيعات', en: 'Active sales days' },
  dashboardStudioCompareTitle: { ar: 'مقارنة سريعة', en: 'Quick compare' },
  dashboardStudioCompareHint: {
    ar: 'الفرق النسبي مقارنةً بالفترة المرجعية (لا يستبدل تقارير التقارير التفصيلية).',
    en: 'Percent change vs reference period (does not replace detailed reports).',
  },
  dashboardStudioCompareCurrent: { ar: 'الحالي', en: 'Current' },
  dashboardStudioComparePrevious: { ar: 'السابق', en: 'Previous' },
  dashboardStudioCompareDelta: { ar: 'التغيير٪', en: 'Change %' },
  dashboardStudioCompareNoPrevYear: { ar: 'لا تتوفر بيانات السنة السابقة بعد.', en: 'Previous year data not available yet.' },
  dashboardStudioClassicChartsTitle: { ar: 'المخططات التفصيلية', en: 'Detail charts' },
  dashboardStudioInvoiceCount: { ar: 'عدد الفواتير', en: 'Invoice count' },
  dashboardStudioScope: { ar: 'النطاق', en: 'Scope' },
  dashboardStudioCsvScope: { ar: 'النطاق', en: 'Scope' },
  dashboardStudioCsvMetric: { ar: 'المؤشر', en: 'Metric' },
  dashboardStudioCsvValue: { ar: 'القيمة', en: 'Value' },
  dashboardStudioCsvPctOfSales: { ar: '٪ من المبيعات', en: '% of sales' },

  dashboardWeeklySalesTitle: {
    ar: 'متوسط المبيعات اليومية حسب أسبوع الشهر',
    en: 'Daily sales average by week of month',
  },
  dashboardWeeklySalesFormulaNote: {
    ar: 'كل أسبوع معزول: 1–7 ثم 8–14، إلخ؛ المتوسط = مجموع الجزء ÷ عدد أيامه في الشهر.',
    en: 'Each week is isolated (1–7, then 8–14, …); average = slice total ÷ days in that slice.',
  },
  dashboardWeeklySalesAvgDailyShort: { ar: 'متوسط يومي', en: 'Daily avg' },
  dashboardWeeklySalesPeriodMainHeader: { ar: 'الفترة الأولى', en: 'Primary period' },
  dashboardWeeklySalesPeriodCompareHeader: { ar: 'فترة المقارنة', en: 'Comparison period' },
  dashboardWeeklySalesPeriodAColumn: { ar: 'شهر العمود الأول', en: 'First column month' },
  dashboardWeeklySalesPeriodAYear: { ar: 'سنة العمود الأول', en: 'First column year' },
  dashboardWeeklySalesPeriodBColumn: { ar: 'شهر عمود المقارنة', en: 'Comparison column month' },
  dashboardWeeklySalesPeriodBYear: { ar: 'سنة عمود المقارنة', en: 'Comparison column year' },
  dashboardWeeklySalesWeekCol: { ar: 'الفترة', en: 'Period' },
  dashboardWeeklySalesWeekRange: {
    ar: 'أسبوع {n} ({from}–{to})',
    en: 'Week {n} ({from}–{to})',
  },
  dashboardWeeklySalesDelta: { ar: 'التغير ٪', en: 'Change %' },
};

    ...
    <ScreenShell variant="data" className="min-w-0">
      ...
      {companyId && tabs.length > 0 && (
        ...
        >
          ...
          {!bootstrapQuery.isLoading && activeTab === 'requests' && <OrdersV4DocumentsTab companyId={companyId} documentType="purchase" startDate={dateFilter.startDate} endDate={dateFilter.endDate} bootstrap={bootstrapQuery.data} canReport={canReport} canCreate={canCreatePurchase} canReverse={canDelete} canUndoReverse={isOwner} canReopen={canReceive} reopenAsCashier={!isOwner && canReceive} canReceive={canReceive} companyName={companyName} companyLogoUrl={companyLogoUrl} />}

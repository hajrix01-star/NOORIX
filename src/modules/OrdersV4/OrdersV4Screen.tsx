...
export default function OrdersV4Screen() {
  ...
  const tabs = useMemo<Array<{ id: TabId; label: string }>>(() => {
    ...
    if (canRead || canCreatePurchase || canReceive) result.push({ id: 'requests', label: lang === 'ar' ? 'الطلبات' : 'Requests' });
    ...
    return result;
  }, [canCreatePurchase, canCreateRegistration, canInventoryWrite, canRead, canReceive, canReport, canWrite, lang]);
    ...
    <ScreenShell variant="data" className="min-w-0">
      ...
      {companyId && tabs.length > 0 && (
        ...
        >
          ...
          {!bootstrapQuery.isLoading && activeTab === 'requests' && <OrdersV4DocumentsTab companyId={companyId} documentType="purchase" startDate={dateFilter.startDate} endDate={dateFilter.endDate} bootstrap={bootstrapQuery.data} canReport={canReport} canCreate={canCreatePurchase} canReverse={canDelete} canUndoReverse={isOwner} canReopen={canReceive} ownerReopenForStaff={isOwner} reopenAsCashier={!isOwner && canReceive} canReceive={canReceive} allowFlexibleDocumentDates={isOwner} companyName={companyName} companyLogoUrl={companyLogoUrl} />}
          {!bootstrapQuery.isLoading && activeTab === 'registration' && <OrdersV4DocumentsTab companyId={companyId} documentType="registration" startDate={isInternalRegistrationStaffView ? registrationPresentation.startDate : dateFilter.startDate} endDate={isInternalRegistrationStaffView ? registrationPresentation.endDate : dateFilter.endDate} bootstrap={bootstrapQuery.data} canReport={canReport} canCreate={canCreateRegistration} canReverse={canDelete} canUndoReverse={isOwner} canCorrectRegistration={isOwner} showOverviewCards={!isInternalRegistrationStaffView} historyWindowDays={isInternalRegistrationStaffView ? 7 : undefined} allowFlexibleDocumentDates={isOwner} companyName={companyName} companyLogoUrl={companyLogoUrl} />}

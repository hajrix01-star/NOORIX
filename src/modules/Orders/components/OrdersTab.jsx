...
function buildWhatsAppText(order, t) {
  const lines = (order.items || []).map((it) => {
    ...
    return `${name}${variantPart}: ${it.quantity} × ${fmt(it.unitPrice ?? 0, 2)} = ${fmt(it.amount ?? 0, 2)} SR`;
  }).join('\n');
...
function buildOrderPrintHtml(order, companyName, t, fmt, formatSaudiDate) {
  ...
  const rows = items.map((it) => {
    ...
    return `<tr><td style="padding:8px 10px;text-align:right;border:1px solid #ddd">${name}</td><td style="padding:8px 10px;text-align:center;border:1px solid #ddd">${it.quantity}</td><td style="padding:8px 10px;text-align:right;border:1px solid #ddd">${fmt(it.unitPrice ?? 0, 2)} SR</td><td style="padding:8px 10px;text-align:right;border:1px solid #ddd;font-weight:600">${fmt(it.amount ?? 0, 2)} SR</td></tr>`;
  }).join('');
  ...
  const pettyRow = order.orderType === 'external' && order.pettyCashAmount != null
    ? `<div style="margin-bottom:8px"><strong>${t('ordersPettyCashGiven')}:</strong> ${fmt(order.pettyCashAmount ?? 0, 2)} SR</div>`
    : '';
...
<table><thead><tr><th style="text-align:right">${t('product')}</th><th style="text-align:center">${t('quantity')}</th><th style="text-align:right">${t('unitPrice')}</th><th style="text-align:right">${t('total')}</th></tr></thead><tbody>${rows}</tbody><tfoot><tr class="total-row"><td colspan="3" style="text-align:right;padding:10px">${t('total')}</td><td style="padding:10px">${fmt(order.totalAmount ?? 0, 2)} SR</td></tr></tfoot></table>
</body></html>`;
}
  ...
  const ordersColumns = useMemo(
    () => [
      ...
      },
      {
        ...
        render: (v, o) =>
          o.orderType === 'external' && v != null ? (
            <span className="nx-cell-num nx-cell-num--blue whitespace-nowrap">{fmt(Number(v), 2)} SR</span>
          ) : (
      ...
      },
      {
        ...
        align: 'center',
        shrink: true,
        render: (v) => <span className="nx-cell-num font-bold whitespace-nowrap">{fmt(Number(v ?? 0), 2)} SR</span>,
      },
      {
        key: 'id',
        ...
        <div className="flex flex-col gap-2">
          ...
          <div className="grid grid-cols-2 gap-2 rounded-lg bg-noorix-bg-muted py-2 px-2.5">
            ...
            <div>
              ...
              <div className="text-[13px] font-bold nx-font-numbers text-noorix-navy">{fmt(Number(o.totalAmount ?? 0), 2)} SR</div>
            </div>
            {pettyGiven != null && (
              <div>
                <div className="text-[10px] text-noorix-muted mb-0.5">{t('ordersPettyCashGiven')}</div>
                <div className="text-[13px] nx-font-numbers text-noorix-blue">{fmt(pettyGiven, 2)} SR</div>
              </div>
            )}
            {cumRem != null && (
              <div>
    ...
    try {
      ...
      if (rows.length === 0) {
        ...
        rows.push({
          ...
          [t('quantity')]: '',
          [t('unitPrice')]: '',
          [t('total')]: t('total') + ': ' + fmt(order.totalAmount ?? 0, 2) + ' SR',
        });
      }
        ...
        badge={
          <div className="noorix-print-hide flex flex-wrap items-center gap-2 w-full min-w-0">
            ...
            <span className="text-[14px] font-bold nx-font-numbers ms-auto shrink-0">
              {t('ordersFilteredTotal')}: {fmt(filteredTotal, 2)} SR
            </span>
          </div>
        }
      />
        ...
        >
          ...
          <div className="grid gap-4 p-4 bg-noorix-bg-muted border border-noorix-border mb-6 rounded-[10px] grid-cols-[repeat(auto-fill,minmax(140px,1fr))]">
            ...
            {viewingOrder.orderType === 'external' && viewingOrder.pettyCashAmount != null && (
              <div>
                ...
                <div className="nx-cell-num nx-cell-num--blue text-[15px] font-semibold">{fmt(viewingOrder.pettyCashAmount ?? 0, 2)} SR</div>
              </div>
            )}
          </div>
          ...
          <SmartTable
            columns={[
              ...
              { key: 'unitPrice', label: t('unitPrice'), numeric: true, render: (v) => `${fmt(v ?? 0, 2)} SR` },
              { key: 'amount',    label: t('total'),    numeric: true, render: (v) => <span className="font-semibold">{fmt(v ?? 0, 2)} SR</span> },
            ]}
            ...
            footerCells={
              <>
                ...
                <td className="nx-cell-num font-bold text-[15px]" style={{ padding: '10px 16px', textAlign: 'right' }}>{fmt(viewingOrder.totalAmount ?? 0, 2)} SR</td>
              </>
            }
          />
        </AdaptiveSheet>

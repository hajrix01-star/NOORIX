...
import type { DigestHistoryDay, DigestHistoryItem } from '../../../types/api';

type DisplayLang = 'ar' | 'en';

/** بناء نص واتساب من بيانات اليوم */
function buildResendText(day: DigestHistoryDay, lang: DisplayLang): string {
...
function DayCard({ day, displayLang }: { day: DigestHistoryDay; displayLang: DisplayLang }) {
  const { t } = useTranslation();
  const [expanded, setExpanded] = useState(false);

  const totalOrders = day.sections.reduce((sum, section) => sum + section.ordersCount, 0);

  function itemName(it: DigestHistoryItem): string {
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="noorix-surface-card px-4 py-3 flex flex-wrap items-center gap-3">
        ...
        <span className="text-[12px] text-noorix-muted ms-auto">
          {history.length} {t('digestHistoryDays')}
        </span>
      </div>

...
export default function StaffOrdersScreen() {
  ...
  return (
    <ScreenShell className="min-w-0 pb-[calc(88px+env(safe-area-inset-bottom,0px))]">
        ...
        <>
          ...
          {tab === 'form' && (
            <div className="flex flex-col gap-4 max-w-lg mx-auto w-full">
              <div>
                <label className="block text-[12px] font-semibold text-noorix-muted mb-1">{t('ordersStaffOrderDate')}</label>
                <Input type="date" value={orderDate} onChange={(e: any) => setOrderDate(e.target.value)} className="w-full min-h-[44px] text-[16px]" />
              </div>

              <div>
                <label className="block text-[12px] font-semibold text-noorix-muted mb-1">{t('ordersStaffNotes')}</label>
                <textarea
                  className="w-full min-h-[72px] rounded-lg border border-noorix-border bg-noorix-bg px-3 py-2 text-[16px] leading-snug"
                  value={notes}
              ...
              <div className="rounded-xl border border-noorix-border p-3 bg-noorix-bg-muted/30">
                <div className="text-[12px] font-bold text-noorix-muted mb-2">{t('ordersStaffAddFromCatalog')}</div>
                ...
                <Button type="button" size="sm" variant="ghost" className="mt-2 w-full" onClick={addCustomLine}>
                  + {t('ordersStaffAddCustomLine')}
                </Button>
              </div>

              <div className="flex flex-col gap-3">
                {lines.length === 0 && <div className="text-center text-noorix-muted text-[13px] py-4">{t('ordersSelectProductAndAdd')}</div>}
                {lines.map((row) => (
                  <div key={row.key} className="rounded-xl border border-noorix-border p-3 space-y-2 bg-noorix-bg">
                    {row.mode === 'product' && (
                      <div className="font-semibold text-[14px]">{row.labelHint || '—'}</div>
                    )}
                    {row.mode === 'custom' && (
                      <div className="space-y-2">
                        <Input
                          ...
                          className="w-full min-h-[44px] text-[16px]"
                        />
                        <Input
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      <div>
                        <label className="text-[10px] text-noorix-muted uppercase tracking-wide">{t('ordersStaffQuantity')}</label>
                        <Input
                          inputMode="decimal"
                          value={row.quantity}
                      ...
                      <div>
                        <label className="text-[10px] text-noorix-muted uppercase tracking-wide">{t('ordersStaffUnitPrice')}</label>
                        <Input
                          inputMode="decimal"
                          value={row.unitPrice}
                    ...
                    <Button type="button" size="sm" variant="ghost" className="text-noorix-accent-red w-full" onClick={() => removeLine(row.key)}>
                      {t('ordersStaffRemoveLine')}
                    </Button>
                  </div>
                ))}
              </div>

              <div className="fixed bottom-0 left-0 right-0 p-3 bg-[var(--noorix-bg)]/95 border-t border-noorix-border backdrop-blur-sm flex gap-2 z-20 max-w-lg mx-auto">
                {editingId && (
                  <Button type="button" variant="ghost" size="sm" className="shrink-0" onClick={resetForm}>
                    {t('cancel')}
                  </Button>
                )}
                <Button type="button" variant="primary" size="sm" className="flex-1 min-h-[48px] text-[16px]" disabled={busy} onClick={handleSave}>

...
export function OrdersV3Panel({ title, action, children }: { title: React.ReactNode; action?: React.ReactNode; children: React.ReactNode }) {
  return (
...
export function OrdersV3Kpi({ label, value, tone = 'blue' }: { label: string; value: React.ReactNode; tone?: 'blue' | 'green' | 'amber' | 'red' }) {
...
export function OrdersV3Select(props: React.SelectHTMLAttributes<HTMLSelectElement>) {
  return <InlineSelect {...props} className={`h-9 rounded-lg px-3 text-[13px] ${props.className || ''}`} />;
}

export function OrdersV3Field({ label, children, className = '' }: { label: string; children: React.ReactNode; className?: string }) {
  return (
...
export function OrdersV3QueryState({ loading, error }: { loading: boolean; error?: Error | null }) {
...
export function v3Number(value: unknown, digits = 2): string {
...
export function v3Date(value: string): string {

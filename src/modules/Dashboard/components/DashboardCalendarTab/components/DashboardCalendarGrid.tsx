import React from 'react';
import { DOW_LABELS, DOW_LABELS_AR } from '../constants';
import { fmt } from '../../../../../utils/format';
import type {
  DashboardCalendarDay,
  DashboardSalesMetricWeekdayAverage,
} from '../../../../../types/api/domains/dashboard';
import DashboardCalendarDayCell from './DashboardCalendarDayCell';

export interface DashboardCalendarGridProps {
  year: number;
  month: number;
  lang: string;
  isLoading: boolean;
  daysInMonth: DashboardCalendarDay[];
  weekdaySalesAverages: DashboardSalesMetricWeekdayAverage[];
  isSelectionMode: boolean;
  selectedDates: Set<string>;
  selectedDay: DashboardCalendarDay | null;
  dayNotes: Record<string, string>;
  maxAmount: number;
  t: (key: string, ...args: unknown[]) => string;
  onDayClick: (item: DashboardCalendarDay, isShift: boolean) => void;
}

export default function DashboardCalendarGrid({
  year,
  month,
  lang,
  isLoading,
  daysInMonth,
  weekdaySalesAverages,
  isSelectionMode,
  selectedDates,
  selectedDay,
  dayNotes,
  maxAmount,
  t,
  onDayClick,
}: DashboardCalendarGridProps) {
  if (isLoading) {
    return <div className="text-center text-noorix-muted text-[13px] p-8">{t('loading')}</div>;
  }

  const firstDow = new Date(year, month - 1, 1).getDay();
  const blanks: null[] = Array(firstDow).fill(null);
  const cells: Array<DashboardCalendarDay | null> = [...blanks, ...daysInMonth];

  return (
    <div className="noorix-calendar-grid-scroll">
      <div className="noorix-calendar-grid-scroll-inner">
        <div className="grid gap-1.5 grid-cols-7">
          {([0, 1, 2, 3, 4, 5, 6] as const).map((d) => {
            const avgDaily = weekdaySalesAverages.find((row) => row.dow === d)?.avgDaily ?? null;
            return (
              <div
                key={d}
                className="flex min-w-0 flex-col items-center text-[12px] font-bold text-noorix-muted text-center py-1.5"
                title={`${t('dashboardCalendarWeekdaySalesAvg')}: ${avgDaily == null ? '—' : `${fmt(avgDaily, 0)} SR`}`}
              >
                <span>{lang === 'ar' ? DOW_LABELS_AR[d] : DOW_LABELS[d]}</span>
                <span className="text-[9px] font-medium leading-tight text-noorix-muted">
                  {t('dashboardCalendarWeekdaySalesAvgShort')}
                </span>
                <span className="max-w-full truncate text-[10px] font-bold leading-tight text-[var(--color-nx-sales)] nx-font-numbers">
                  {avgDaily == null ? '—' : fmt(avgDaily, 0)}
                </span>
              </div>
            );
          })}
          {cells.map((item, i) => {
            if (!item) return <div key={`b-${i}`} />;
            return (
              <DashboardCalendarDayCell
                key={item.dateStr}
                item={item}
                isSelectionMode={isSelectionMode}
                selectedDates={selectedDates}
                selectedDay={selectedDay}
                dayNotes={dayNotes}
                maxAmount={maxAmount}
                t={t}
                onDayClick={onDayClick}
              />
            );
          })}
        </div>
      </div>
    </div>
  );
}
